  ...
  try {
    await Helpers.callWithStopwatch(
      () async {
        ...
        final deletedAssetFolderName = '$flutterProjectPath/deleted_assets_';
        ...
        final projectFiles = Helpers.callWithStopwatch(
          ...
          whenDone: (milliseconds) => write('collect projectFiles: $milliseconds ms'),
        );
          ...
          () {
            ...
            for (final path in projectFilesPaths) {
              ...
              for (final assetFileName in assetsFilesNamesWithoutAlreadyUsed) {
                /// get the image name without '.' format i.e. .png, .svg
                String localAssetFileName = '';
                if (assetFileName.contains('.')) {
                  localAssetFileName.toString().split('.')[0];
                } else {
                  localAssetFileName = assetFileName;
                }

                if (fileString.contains(localAssetFileName)) {
        ...
        final unUsedAssets = assetsFiles.entries.where((element) => !usedAssetsNames.contains(element.key));
        final unUsedAssetsNames = unUsedAssets.map((asset) => asset.key);
        final unUsedAssetsPaths = unUsedAssets.map((asset) => asset.value);

        write('Assets Files count: ${assetsFiles.length}', colorType: ConsoleColorType.attention);
        write('Used Assets count: ${usedAssetsNames.length}', colorType: ConsoleColorType.attention);
        write('UnUsed Assets count: ${unUsedAssetsNames.length}', colorType: ConsoleColorType.attention);
